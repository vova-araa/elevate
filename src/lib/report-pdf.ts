...
function drawReport(doc: jsPDF, r: ReportRow, startY: number, marginX: number, contentW: number): number {
  let y = startY;
  ...
  doc.text(`${r.report_type.toUpperCase()}  •  ${fmt(r.period_start)} → ${fmt(r.period_end)}`, marginX, y);
  y += 6;
  doc.setTextColor(0);
  ...
  if (r.summary) {
    ...
    doc.text("Samenvatting", marginX, y); y += 5;
  ...
  if (r.highlights) {
    y = ensureSpace(doc, y, 20);
    doc.setFont("helvetica", "bold").setFontSize(10);
    doc.text("Highlights", marginX, y); y += 5;
    doc.setFont("helvetica", "normal").setFontSize(10);
...
function header(doc: jsPDF, clientName: string, subtitle: string, marginX: number, contentW: number): number {
...
export function exportReportPdf(clientName: string, r: ReportRow) {
  ...
  const y = header(doc, clientName, `Rapportage • ${fmt(r.period_start)} → ${fmt(r.period_end)}`, marginX, contentW);
...
export function exportAllReportsPdf(clientName: string, reports: ReportRow[]) {
  ...
  const marginX = 15;
  const contentW = doc.internal.pageSize.getWidth() - marginX * 2;
  let y = header(doc, clientName, `Alle rapportages (${reports.length}) • ${new Date().toLocaleDateString("nl-NL")}`, marginX, contentW);
