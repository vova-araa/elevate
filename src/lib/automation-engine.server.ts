...
export async function dispatchEvent(event: string, payload: Record<string, unknown>, clientId?: string | null) {
  const sb = admin();
  ...
  await Promise.all(filtered.map(async (ep: any) => {
    const body = JSON.stringify({ event, timestamp: new Date().toISOString(), data: payload });
    const headers: Record<string, string> = { "Content-Type": "application/json", "X-Elevate-Event": event };
    ...
    try {
      ...
      await sb.from("webhook_deliveries").insert({
        endpoint_id: ep.id, event, payload, status_code: res.status,
      ...
      await sb.from("webhook_deliveries").insert({
        endpoint_id: ep.id, event, payload, error_message: String(err?.message ?? err),
      });
      await sb.from("webhook_endpoints").update({
...
async function executeAction(rule: any, context: Record<string, unknown> = {}) {
  ...
  try {
    if (rule.action_type === "create_notification") {
      ...
      const userIds: string[] = cfg.user_id ? [cfg.user_id] : (admins ?? []).map((a: any) => a.user_id);
      ...
      if (cfg.url) {
        const body = JSON.stringify({ rule: rule.name, timestamp: new Date().toISOString(), context });
        await fetch(cfg.url, { method: "POST", headers: { "Content-Type": "application/json" }, body }).catch(() => null);
      }
    ...
    await sb.from("automation_runs").insert({ rule_id: rule.id, status: "success", payload: context });
    return true;
  } catch (err: any) {
    await sb.from("automation_runs").insert({
      rule_id: rule.id, status: "error", payload: context, error_message: String(err?.message ?? err),
    });
    return false;
  }
}
...
function isDue(rule: any, now: Date): boolean {
  ...
  const minIntervalMs = freq === "daily" ? 23 * 3600_000 : freq === "weekly" ? 6 * 86400_000 : 27 * 86400_000;
  ...
  if (freq === "weekly" && cfg.day_of_week !== undefined && now.getUTCDay() !== Number(cfg.day_of_week)) return false;
  if (freq === "monthly" && cfg.day_of_month !== undefined && now.getUTCDate() !== Number(cfg.day_of_month)) return false;
  return true;
}
...
export async function authenticateApiKey(req: Request): Promise<{ ok: boolean; key?: any; error?: string }> {
  ...
  const sb = admin();
  const { data } = await sb.from("api_keys").select("*").eq("key_prefix", prefix).eq("is_active", true).maybeSingle();
