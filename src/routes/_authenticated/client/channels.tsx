...
import {
  Instagram, Linkedin, Youtube, Facebook, Music2, CheckCircle2, Loader2, Link2, X,
} from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/client/channels")({ component: ChannelsPage });

type Platform = "instagram" | "tiktok" | "linkedin" | "youtube" | "facebook";

const PLATFORMS: { id: Platform; label: string; Icon: any; tint: string }[] = [
  { id: "instagram", label: "Instagram", Icon: Instagram, tint: "from-fuchsia-500/15 to-rose-500/10 text-rose-300" },
  { id: "tiktok",    label: "TikTok",    Icon: Music2,    tint: "from-cyan-500/15 to-pink-500/10 text-cyan-300" },
  { id: "linkedin",  label: "LinkedIn",  Icon: Linkedin,  tint: "from-sky-500/15 to-blue-500/10 text-sky-300" },
  { id: "youtube",   label: "YouTube",   Icon: Youtube,   tint: "from-red-500/15 to-orange-500/10 text-red-300" },
  { id: "facebook",  label: "Facebook",  Icon: Facebook,  tint: "from-indigo-500/15 to-blue-500/10 text-indigo-300" },
];
...
function ChannelsPage() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "social_connections", filter: `client_id=eq.${data.clientId}` },
        () => qc.invalidateQueries({ queryKey: ["client-channels"] }))
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "clients", filter: `id=eq.${data.clientId}` },
        () => qc.invalidateQueries({ queryKey: ["client-channels"] }))
      .subscribe();
  ...
  const syncAfterConnect = async (platform: Platform, notify = false) => {
    ...
    await refetch();
    if (notify) toast[result?.connected ? "success" : "info"](result?.connected ? "Account gekoppeld" : (result?.reason ?? "Nog geen nieuwe koppeling gevonden"));
  };
  ...
  const connectMut = useMutation({
    ...
    onSuccess: (res: any) => {
      setConfirm(null);
      if (res?.external) {
        toast.info("Rond de autorisatie af in het nieuwe tabblad. De status wordt automatisch ververst.");
        [4000, 12000, 30000].forEach((ms) => window.setTimeout(() => syncAfterConnect(res.platform), ms));
      } else {
  ...
  const disconnectMut = useMutation({
    ...
    onSuccess: () => { toast.success("Ontkoppeld"); refetch(); },
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">Koppel je social-accounts. Dit duurt ongeveer 30 seconden per platform.</p>
      </header>
            ...
            >
              ...
              <div className="mt-4 flex items-center gap-2">
                {connected ? (
                  ...
                  >
                    {disconnectMut.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Ontkoppelen"}
                  </button>
                ) : (
                  <button
      ...
      {confirm && (() => {
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setConfirm(null)}>
            <div
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setConfirm(null)} className="h-8 w-8 rounded-full grid place-items-center text-muted-foreground hover:bg-accent/30">
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Je wordt doorgestuurd naar {meta.label} om toegang te geven. Dit duurt maar 30 seconden.
                </p>
              </div>
