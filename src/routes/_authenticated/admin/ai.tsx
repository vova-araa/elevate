...
import { Loader2, Sparkles, Send, Calendar, ListTodo, FileText, Bot, User as UserIcon, Image as ImageIcon, X, Paperclip } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/admin/ai")({ component: AiPage });

type Msg = { role: "user" | "assistant"; content: string; actions?: { tool: string; result: any }[] };

const QUICK = [
  { id: "caption", label: "Caption", Icon: Sparkles, prompt: "Schrijf een caption voor Instagram en LinkedIn over: " },
  { id: "schedule", label: "Inplannen", Icon: Calendar, prompt: "Plan een post in voor morgen 10:00 met deze tekst: " },
...
function AiPage() {
  ...
  const scheduleWithMedia = () => {
    ...
    sessionStorage.setItem(
      ...
      JSON.stringify({ media, caption: messages.filter((m) => m.role === "assistant").slice(-1)[0]?.content ?? "" }),
    );
  ...
  return (
    <div className="max-w-3xl flex flex-col h-[calc(100vh-9rem)]">
      <header className="mb-3 flex items-center gap-3">
        <div className="h-10 w-10 rounded-full bg-gradient-gold grid place-items-center text-primary-foreground">
          <Bot className="h-5 w-5" />
        </div>
        <div>
          <h1 className="font-display text-2xl leading-tight">AI Assistent</h1>
          <p className="text-xs text-muted-foreground">
            Captions, planning, taken & notities — {activeClient?.name ?? "selecteer een klant"}
          </p>
        </div>
      </header>
              ...
              <div className="flex flex-wrap gap-2">
                {media.map((m, i) => (
                  <div key={i} className="relative group">
                    <img src={m.url} alt={m.name} className="h-24 w-24 object-cover rounded-lg border border-gold/20" />
                    <button
