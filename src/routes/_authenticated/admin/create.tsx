...
import { generateContentIdeas, generateCaption, generateHooks, generateHashtags } from "@/lib/planner.functions";
import { cn } from "@/lib/utils";
import { toast } from "sonner";
import { z } from "zod";
import {
  Sparkles, Loader2, Instagram, Music2, Linkedin, Youtube, Facebook, Copy, Send,
  Lightbulb, MessageSquareQuote, Hash, Wand2, ArrowRight, CheckCircle2, Plus, X,
} from "lucide-react";
...
function CreatePage() {
  ...
  const { data: clients } = useQuery({
    ...
    queryFn: async () => (await supabase.from("clients").select("id,name,industry,description,brand_color").order("name")).data ?? [],
  });
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-xl">
            Genereer ideeën, captions, hooks en hashtags op basis van de klant en plan ze door naar de Planner.
          </p>
        </div>
        <select
          value={activeId ?? ""}
    ...
    <div className="grid lg:grid-cols-[340px_1fr] gap-6">
      <div className="glass-strong rounded-2xl p-5 space-y-4 h-fit">
        ...
        <Field label="Platforms">
          <div className="flex flex-wrap gap-1.5">
            {PLATFORMS.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => setPlatforms((cur) => cur.includes(p.id) ? cur.filter((x) => x !== p.id) : [...cur, p.id])}
                  className={cn(
                    "rounded-full border px-2.5 py-1 text-xs inline-flex items-center gap-1 transition",
                    active ? "bg-gold/15 text-gold border-gold/40" : "border-border/40 text-muted-foreground",
                  )}>
        ...
        <Field label="Content pijlers">
          <textarea value={pillars} onChange={(e) => setPillars(e.target.value)} rows={2}
        ...
        <Field label={`Aantal ideeën: ${count}`}>
          <input type="range" min={3} max={15} value={count} onChange={(e) => setCount(Number(e.target.value))} className="w-full accent-gold" />
        </Field>
        <button onClick={run} disabled={loading}
          ...
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />} Genereer ideeën
        </button>
      </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    {meta && <span className="text-[11px] inline-flex items-center gap-1 rounded-full border border-gold/30 bg-gold/5 px-2 py-0.5"><meta.Icon className="h-3 w-3" /> {meta.label}</span>}
                    <span className="text-[11px] rounded-full border border-border/40 px-2 py-0.5 text-muted-foreground">{idea.format}</span>
                    {idea.pillar && <span className="text-[11px] rounded-full border border-border/40 px-2 py-0.5 text-muted-foreground">{idea.pillar}</span>}
                  </div>
                  <h3 className="font-display text-xl">{idea.title}</h3>
                  <p className="text-sm text-gold mt-1.5 italic">"{idea.hook}"</p>
                  <p className="text-sm text-muted-foreground mt-2 whitespace-pre-wrap">{idea.description}</p>
                </div>
              </div>
              <div className="mt-4 flex flex-wrap items-center gap-2 pt-4 border-t border-border/30">
                <input type="datetime-local"
                  defaultValue={(() => { const d = new Date(); d.setDate(d.getDate() + 1); d.setHours(10,0,0,0); const tz = d.getTimezoneOffset(); return new Date(d.getTime() - tz*60000).toISOString().slice(0,16); })()}
                  id={`dt-${i}`}
                ...
                <button onClick={() => {
                  ...
                  {schedulingId === String(i) ? <Loader2 className="h-3 w-3 animate-spin" /> : <Send className="h-3 w-3" />}
                  Plan in als concept
                </button>
                <button onClick={() => navigator.clipboard.writeText(`${idea.hook}\n\n${idea.description}`).then(() => toast.success("Gekopieerd"))}
...
function CaptionTab({ client, userId }: { client: any; userId?: string }) {
  ...
  async function run() {
    ...
    try {
      const res = await captionFn({
        data: { brief, platform, tone: tone || undefined, brand: `${client.name}${client.industry ? " — " + client.industry : ""}` },
      });
  ...
  async function schedule(when: string) {
    ...
    try {
      ...
      const { error } = await supabase.from("scheduled_posts").insert({
        client_id: client.id, platform, caption: full,
        scheduled_at: new Date(when).toISOString(),
        status: "draft", created_by: userId ?? null,
      } as any);
      if (error) throw error;
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      <div className="glass-strong rounded-2xl p-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {PLATFORMS.map((p) => (
              <button key={p.id} onClick={() => setPlatform(p.id)}
                className={cn("rounded-full border px-2.5 py-1 text-xs inline-flex items-center gap-1 transition",
                  platform === p.id ? "bg-gold/15 text-gold border-gold/40" : "border-border/40 text-muted-foreground")}>
                <p.Icon className="h-3 w-3" /> {p.label}
              </button>
            ))}
          </div>
        ...
        <button onClick={run} disabled={loading}
          className="w-full rounded-full bg-gradient-gold text-primary-foreground px-4 py-2.5 text-sm font-medium inline-flex items-center justify-center gap-2 disabled:opacity-60">
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />} Genereer caption
        </button>
      </div>
      ...
      <div className="glass-strong rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {caption && <span className="text-[10px] text-muted-foreground">{fullText.length} tekens</span>}
        </div>
        ...
        {hashtags.length > 0 && (
          <div>
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1.5">Hashtags</div>
            <div className="flex flex-wrap gap-1.5">
              {hashtags.map((h, i) => (
                <span key={i} className="text-[11px] rounded-full border border-gold/30 bg-gold/5 px-2 py-0.5">{h}</span>
              ))}
            </div>
          </div>
        )}
        {caption && (
          <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-border/30">
            <input type="datetime-local" id="cap-dt"
              defaultValue={(() => { const d = new Date(); d.setDate(d.getDate()+1); d.setHours(10,0,0,0); const tz=d.getTimezoneOffset(); return new Date(d.getTime()-tz*60000).toISOString().slice(0,16); })()}
              className="rounded-lg bg-input/60 hairline px-3 py-1.5 text-xs outline-none focus:ring-2 focus:ring-gold/40" />
            <button onClick={() => { const el = document.getElementById("cap-dt") as HTMLInputElement; if (el?.value) schedule(el.value); }} disabled={scheduling}
              className="rounded-full bg-gradient-gold text-primary-foreground px-3 py-1.5 text-xs inline-flex items-center gap-1.5">
              {scheduling ? <Loader2 className="h-3 w-3 animate-spin" /> : <Send className="h-3 w-3" />} Plan in als concept
            </button>
            <button onClick={() => navigator.clipboard.writeText(fullText).then(() => toast.success("Gekopieerd"))}
              className="rounded-full glass px-3 py-1.5 text-xs inline-flex items-center gap-1.5 hover:bg-accent/30 ml-auto">
              <Copy className="h-3 w-3" /> Kopieer
            </button>
          </div>
  ...
  return (
    <div className="grid lg:grid-cols-[340px_1fr] gap-6">
      <div className="glass-strong rounded-2xl p-5 space-y-4 h-fit">
        ...
        <Field label="Platform">
          <div className="flex flex-wrap gap-1.5">
            {PLATFORMS.map((p) => (
              <button key={p.id} onClick={() => setPlatform(p.id)}
                className={cn("rounded-full border px-2.5 py-1 text-xs inline-flex items-center gap-1",
                  platform === p.id ? "bg-gold/15 text-gold border-gold/40" : "border-border/40 text-muted-foreground")}>
                <p.Icon className="h-3 w-3" /> {p.label}
              </button>
            ))}
        ...
        <button onClick={run} disabled={loading}
          className="w-full rounded-full bg-gradient-gold text-primary-foreground px-4 py-2.5 text-sm font-medium inline-flex items-center justify-center gap-2 disabled:opacity-60">
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Wand2 className="h-4 w-4" />} Genereer hooks
        </button>
      </div>
      <div className="space-y-2">
        ...
        {loading && <div className="glass-strong rounded-2xl p-10 text-center"><Loader2 className="h-6 w-6 animate-spin text-gold mx-auto" /></div>}
        {hooks.map((h, i) => (
          <button key={i} onClick={() => navigator.clipboard.writeText(h).then(() => toast.success("Gekopieerd"))}
...
function HashtagsTab() {
  ...
  const [loading, setLoading] = useState(false);
  const [groups, setGroups] = useState<{ big: string[]; medium: string[]; niche: string[] } | null>(null);
  ...
  return (
    <div className="grid lg:grid-cols-[340px_1fr] gap-6">
      <div className="glass-strong rounded-2xl p-5 space-y-4 h-fit">
        ...
        <Field label="Onderwerp"><textarea value={topic} onChange={(e) => setTopic(e.target.value)} rows={3}
          placeholder="Waar gaat de post over?"
          className="w-full rounded-lg bg-input/60 hairline px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gold/40" /></Field>
        <Field label="Niche (optioneel)"><input value={niche} onChange={(e) => setNiche(e.target.value)}
          placeholder="Bijv. duurzame mode in NL"
          className="w-full rounded-lg bg-input/60 hairline px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gold/40" /></Field>
        <button onClick={run} disabled={loading}
          className="w-full rounded-full bg-gradient-gold text-primary-foreground px-4 py-2.5 text-sm font-medium inline-flex items-center justify-center gap-2 disabled:opacity-60">
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Hash className="h-4 w-4" />} Genereer hashtags
        </button>
      </div>
        ...
        {groups && (
          <>
            <div className="flex justify-end">
              <button onClick={copyAll} className="rounded-full glass px-3 py-1.5 text-xs inline-flex items-center gap-1.5 hover:bg-accent/30">
                <Copy className="h-3 w-3" /> Kopieer alle
              </button>
            </div>
            <HashGroup label="Groot bereik" tags={groups.big} tone="border-fuchsia-400/30 bg-fuchsia-500/5 text-fuchsia-200" />
            <HashGroup label="Middel" tags={groups.medium} tone="border-sky-400/30 bg-sky-500/5 text-sky-200" />
            <HashGroup label="Niche" tags={groups.niche} tone="border-emerald-400/30 bg-emerald-500/5 text-emerald-200" />
          </>
        )}
      </div>
    </div>
  ...
  return (
    <div className="glass-strong rounded-2xl p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={() => navigator.clipboard.writeText(tags.join(" ")).then(() => toast.success("Gekopieerd"))}
          className="text-[11px] text-muted-foreground hover:text-foreground inline-flex items-center gap-1"><Copy className="h-3 w-3" /> Kopieer</button>
      </div>
      <div className="flex flex-wrap gap-1.5">
        {tags.map((t, i) => (
          <span key={i} className={cn("text-[11px] rounded-full border px-2 py-0.5", tone)}>{t}</span>
        ))}
      </div>
    </div>
  );
