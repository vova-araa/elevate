...
import {
  Loader2, Building2, Users, Bell, Link2, Plus, Trash2, Save,
  Instagram, Music2, Linkedin, Youtube, Facebook, ShieldCheck, UserPlus, Zap, Plug, Key,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { initPostizConnect, disconnectPostizChannel, syncPostizConnection, claimPostizIntegration } from "@/lib/postiz-connect.functions";
...
function SettingsPage() {
  ...
  const { data: clients } = useQuery({
    ...
    queryFn: async () => (await supabase.from("clients").select("id,name,brand_color,logo_url").order("name")).data ?? [],
  });
  ...
  if (!clientId && activeId) {
    navigate({ to: "/admin/settings", search: { clientId: activeId, tab: activeTab }, replace: true });
  }
  ...
  if (clients.length === 0) {
    return <div className="glass-strong rounded-xl p-8 text-center text-muted-foreground">
      Voeg eerst een klant toe via <Link to="/admin/clients/new" className="text-gold underline">Klanten</Link>.
    </div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <select value={activeId ?? ""}
          onChange={(e) => navigate({ to: "/admin/settings", search: { clientId: e.target.value, tab: activeTab } })}
      ...
      <div className="flex flex-wrap items-center gap-1 border-b border-gold/10">
        {tabs.map((t) => (
          <button key={t.k}
            onClick={() => navigate({ to: "/admin/settings", search: { clientId: activeId, tab: t.k } })}
            className={cn("flex items-center gap-2 px-4 py-2.5 text-sm border-b-2 -mb-px transition",
              activeTab === t.k ? "border-gold text-gold" : "border-transparent text-muted-foreground hover:text-foreground")}>
      ...
      <div className="flex flex-wrap gap-2">
        <span className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground self-center mr-2">Beheer</span>
        <Link to="/admin/users" className="flex items-center gap-2 rounded-full border border-gold/20 bg-background/40 px-3 py-1.5 text-xs text-muted-foreground hover:text-gold hover:border-gold/40 transition">
          <Users className="h-3.5 w-3.5" /> Gebruikers
        </Link>
        <Link to="/admin/api-keys" className="flex items-center gap-2 rounded-full border border-gold/20 bg-background/40 px-3 py-1.5 text-xs text-muted-foreground hover:text-gold hover:border-gold/40 transition">
          <Key className="h-3.5 w-3.5" /> API sleutels
        </Link>
        <Link to="/admin/webhooks" className="flex items-center gap-2 rounded-full border border-gold/20 bg-background/40 px-3 py-1.5 text-xs text-muted-foreground hover:text-gold hover:border-gold/40 transition">
          <Plug className="h-3.5 w-3.5" /> Kanalen
        </Link>
        <Link to="/admin/automations" className="flex items-center gap-2 rounded-full border border-gold/20 bg-background/40 px-3 py-1.5 text-xs text-muted-foreground hover:text-gold hover:border-gold/40 transition">
...
function BrandTab({ clientId }: { clientId: string }) {
  ...
  async function save() {
    const { error } = await supabase.from("clients").update({
      ...
      website: form.website, brand_color: form.brand_color, notes: form.notes,
    }).eq("id", clientId);
  ...
  return (
    <div className="glass-strong rounded-xl p-6 space-y-4 max-w-3xl">
      <div>
        <label className="text-xs uppercase tracking-wider text-muted-foreground">Logo / Avatar</label>
        <div className="mt-2 flex items-center gap-4">
          {form.logo_url ? (
            <img src={form.logo_url} alt={form.name} className="h-16 w-16 rounded-full object-cover border border-gold/20" />
          ) : (
            <div className="h-16 w-16 rounded-full flex items-center justify-center font-display text-2xl text-primary-foreground"
          ...
          <div className="flex flex-col gap-2">
            ...
            {form.logo_url && (
              <button onClick={removeLogo} className="text-xs text-muted-foreground hover:text-destructive text-left">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="Bedrijfsnaam" value={form.name} onChange={(v: string) => setForm({ ...form, name: v })} />
        <Field label="Branche" value={form.industry} onChange={(v: string) => setForm({ ...form, industry: v })} />
        <Field label="Website" value={form.website} onChange={(v: string) => setForm({ ...form, website: v })} placeholder="https://..." />
        <div>
          <label className="text-xs uppercase tracking-wider text-muted-foreground">Brandkleur (fallback)</label>
          <div className="flex gap-2 mt-1">
            <input type="color" value={form.brand_color || "#D4B97A"}
      ...
      <div>
        <label className="text-xs uppercase tracking-wider text-muted-foreground">Korte beschrijving</label>
        <textarea value={form.description || ""} onChange={(e) => setForm({ ...form, description: e.target.value })}
          rows={3} className="mt-1 w-full rounded-lg border border-gold/20 bg-background/60 px-3 py-2 text-sm" />
      </div>
      <div>
        <label className="text-xs uppercase tracking-wider text-muted-foreground">Interne notities</label>
        <textarea value={form.notes || ""} onChange={(e) => setForm({ ...form, notes: e.target.value })}
          rows={3} className="mt-1 w-full rounded-lg border border-gold/20 bg-background/60 px-3 py-2 text-sm" />
      </div>
      <button onClick={save} className="rounded-lg bg-gradient-gold px-4 py-2.5 text-sm font-medium text-primary-foreground inline-flex items-center gap-2">
...
function SocialTab({ clientId }: { clientId: string }) {
  ...
  const { data: conns } = useQuery({
    ...
    queryFn: async () => (await supabase.from("social_connections").select("*").eq("client_id", clientId)).data ?? [],
  });
    ...
    try {
      ...
      if (showToast) {
        toast[result?.connected ? "success" : "info"](
          result?.connected ? `${platform} gekoppeld${result?.handle ? `: ${result.handle}` : ""}` : (result?.reason ?? "Nog geen nieuwe Postiz-koppeling gevonden"),
        );
      }
      return result;
    } catch (e: any) {
  ...
  async function connectPlatform(platform: string) {
    ...
    try {
      ...
      if (!opened) throw new Error("De browser blokkeerde het Postiz-tabblad. Sta pop-ups toe en probeer opnieuw.");
      ...
      [4000, 9000, 18000, 35000].forEach((ms) => window.setTimeout(() => syncPlatform(platform, false), ms));
    } catch (e: any) {
    ...
    <div className="space-y-3">
      <div className="glass-strong rounded-xl p-4 text-xs text-muted-foreground flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          Platforms worden gekoppeld via je Postiz workspace. Na autoriseren synct deze pagina automatisch.
        </div>
        <button
          onClick={async () => {
            setBusy("__all__");
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {PLATFORMS.map((p) => {
          ...
          const otherClient = !conn && !ownIntegration && claimable.length === 0
            ? platformIntegrations.find((i) => i.assignedClientId && i.assignedClientId !== clientId)
            : null;
          const Icon = p.icon;
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-sm font-medium flex items-center gap-1.5">
                      ...
                      {isLinked && <span className="text-[10px] rounded-full bg-emerald-500/15 text-emerald-400 px-1.5 py-0.5">gekoppeld</span>}
                    </div>
                      ...
                      {isLinked ? (
                        <>
                          ...
                          {typeof conn?.follower_count === "number" && (
                            <span className="ml-1 text-muted-foreground/70">· {conn.follower_count.toLocaleString("nl-NL")} volgers</span>
                          )}
                        </>
                      ) : otherClient ? (
                        <span className="text-amber-400/80">In gebruik door {otherClient.assignedClientName ?? "andere klant"}</span>
                      ) : claimable.length > 0 ? (
                        <>Postiz-account beschikbaar ({claimable[0].username ?? claimable[0].name ?? "naamloos"})</>
                      ) : "Niet gekoppeld"}
                    </div>
                  </div>
                </div>
                {isLinked ? (
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <button onClick={() => remove(conn)} className="text-muted-foreground hover:text-destructive">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    >
                      ...
                      {claimable.map((i) => (
                        <option key={i.id} value={i.id}>{i.username ?? i.name ?? i.id}</option>
                      ))}
                    </select>
                  </div>
...
function TeamTab({ clientId }: { clientId: string }) {
  ...
  const { data: members } = useQuery({
    ...
    queryFn: async () => {
      const { data: m } = await supabase.from("client_members").select("id,user_id,created_at").eq("client_id", clientId);
      if (!m?.length) return [];
  ...
  async function addMember() {
    ...
    if (!profile) return toast.error("Geen gebruiker met dit e-mailadres gevonden. Laat ze eerst registreren.");
    const { error } = await supabase.from("client_members").insert({ client_id: clientId, user_id: profile.id });
    if (error) return toast.error(error.message);
    // Ensure role exists
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="glass-strong rounded-xl divide-y divide-gold/10">
        {!members ? (
          ...
          <div key={m.id} className="flex items-center gap-3 p-4">
            ...
            <select value={m.roles[0] || "client"} onChange={(e) => updateRole(m.user_id, e.target.value as any)}
      ...
      <div className="text-xs text-muted-foreground flex items-start gap-2">
        ...
        <span><strong>Admin</strong> beheert alles. <strong>Editor</strong> mag content maken en plannen. <strong>Klant viewer</strong> ziet alleen het portaal en kan goedkeuren.</span>
      </div>
    </div>
  );
}
...
function NotificationsTab() {
  ...
  const [form, setForm] = useState<any>({
    ...
    notify_new_message: true, notify_new_upload: true, notify_approval: true,
    notify_publish: true, notify_failure: true, notify_task_assigned: true,
    notify_ai: true, notify_planning: true, notify_automation: true,
  });
  ...
  const channels = [
    { k: "in_app_enabled", label: "In-app meldingen", hint: "Toon meldingen in de bel rechtsboven." },
    { k: "email_enabled", label: "E-mailmeldingen", hint: "Stuur belangrijke meldingen naar mijn e-mail." },
  ];
  const groups = [
    {
    ...
    },
    {
      ...
      items: [
        { k: "notify_planning", label: "Planning wijzigingen", hint: "Items toegevoegd, verplaatst of verwijderd." },
        { k: "notify_approval", label: "Goedkeuringen & feedback", hint: "Klant keurt goed of geeft feedback." },
        { k: "notify_publish", label: "Post gepubliceerd", hint: "Een ingeplande post is live gegaan." },
        { k: "notify_failure", label: "Post mislukt / fout", hint: "Een publicatie of integratie faalt." },
      ],
    },
    {
      title: "AI & automations",
      items: [
        { k: "notify_ai", label: "AI generatie klaar", hint: "Captions, ideeën of beelden zijn klaar." },
        { k: "notify_automation", label: "Automation uitgevoerd", hint: "Een automation-regel is getriggerd." },
        { k: "notify_task_assigned", label: "Taak toegewezen", hint: "Er staat een nieuwe taak voor jou klaar." },
      ],
    },
  ];
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="glass-strong rounded-xl p-5">
        ...
        <div className="space-y-3">
          {channels.map((t) => (
            <Toggle key={t.k} label={t.label} hint={t.hint}
              checked={!!form[t.k]} onChange={(v) => setForm({ ...form, [t.k]: v })} />
          ))}
        </div>
      </div>
  ...
  return (
    <label className="flex items-start justify-between gap-4 cursor-pointer">
      ...
      <button type="button" onClick={() => onChange(!checked)}
        className={cn("relative h-6 w-11 rounded-full transition shrink-0",
          checked ? "bg-gold" : "bg-muted")}>
        <span className={cn("absolute top-0.5 h-5 w-5 rounded-full bg-background transition-all",
