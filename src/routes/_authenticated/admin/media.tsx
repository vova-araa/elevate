...
import { Trash2, Download, Search as SearchIcon, Image as ImageIcon, Video, FileText, Loader2, FolderPlus, Folder, ChevronLeft, Pencil, FolderInput } from "lucide-react";
...
function MediaLibrary() {
  ...
  useEffect(() => { setFolderId(null); }, [clientId]);
  ...
  const { data: uploads, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("uploads").select("*, clients(name)").order("created_at", { ascending: false }).limit(500);
  ...
  const filtered = (uploads ?? []).filter((u: any) => {
    ...
    if (filter === "other") return !u.file_type?.startsWith("image/") && !u.file_type?.startsWith("video/");
    return true;
  });
  ...
  async function createFolder() {
    ...
    if (!name?.trim()) return;
    const { error } = await supabase.from("media_folders").insert({ client_id: clientId, name: name.trim() });
  ...
  async function deleteFolder(f: any) {
    if (!confirm(`Map "${f.name}" verwijderen? Bestanden blijven bewaard en gaan terug naar de hoofdmap.`)) return;
  ...
  async function moveUpload(u: any) {
    ...
    const options = ["(hoofdmap)", ...folders.map((f: any) => f.name)].map((n, i) => `${i}: ${n}`).join("\n");
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-2">Alle beelden, video's en bestanden van klanten op één plek. Maak per bedrijf overzichtelijk mappen aan.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative">
          ...
          <input
            value={q} onChange={(e) => setQ(e.target.value)} placeholder="Zoek bestandsnaam…"
            className="rounded-lg bg-input/60 hairline pl-9 pr-3 py-2 text-sm w-64"
          />
        </div>
        <select value={clientId} onChange={(e) => setClientId(e.target.value)} className="rounded-lg bg-input/60 hairline px-3 py-2 text-sm">
        ...
        {clientId && (
          <button onClick={createFolder} className="flex items-center gap-1.5 rounded-lg bg-gold/15 text-gold hairline px-3 py-2 text-sm hover:bg-gold/25">
      ...
      {clientId && (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            <button onClick={() => setFolderId(null)} className={`flex items-center gap-1 ${folderId ? "text-muted-foreground hover:text-foreground" : "text-gold"}`}>
              {folderId && <ChevronLeft className="h-4 w-4" />}
              <Folder className="h-4 w-4" /> Hoofdmap
            </button>
            {currentFolder && <span className="text-muted-foreground">/ <span className="text-foreground">{currentFolder.name}</span></span>}
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
              {folders.map((f: any) => (
                <div key={f.id} className="group relative">
                  <button onClick={() => setFolderId(f.id)} className="w-full flex items-center gap-2 rounded-xl glass hairline p-3 hover:bg-accent/40 text-left">
                  ...
                  <div className="absolute right-1 top-1 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                    <button onClick={() => renameFolder(f)} className="rounded-md bg-background/80 p-1 hover:bg-accent" title="Hernoemen">
                      <Pencil className="h-3 w-3" />
                    </button>
                    <button onClick={() => deleteFolder(f)} className="rounded-md bg-destructive/80 p-1 text-white hover:bg-destructive" title="Verwijderen">
      ...
      {isLoading ? (
        ...
        <div className="glass rounded-2xl p-12 text-center text-muted-foreground">
          {clientId ? (folderId ? "Deze map is leeg" : "Geen losse bestanden — kies of maak een map") : "Geen media gevonden"}
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {filtered.map((u: any) => <Tile key={u.id} u={u} onDelete={() => remove(u)} onMove={clientId ? () => moveUpload(u) : undefined} />)}
        </div>
      )}
    </div>
  );
...
function Tile({ u, onDelete, onMove }: { u: any; onDelete: () => void; onMove?: () => void }) {
  ...
  return (
    <div className="group relative aspect-square overflow-hidden rounded-xl glass">
      {url && isImage && <img src={url} alt={u.file_name} className="h-full w-full object-cover transition group-hover:scale-105" />}
      ...
      <div className="absolute inset-x-0 top-0 flex items-center justify-between p-2 bg-gradient-to-b from-black/70 to-transparent">
        <span className="text-[10px] uppercase tracking-wider text-white/80 flex items-center gap-1">
          {isImage ? <ImageIcon className="h-3 w-3" /> : isVideo ? <Video className="h-3 w-3" /> : <FileText className="h-3 w-3" />}
          {u.clients?.name ?? "—"}
        </span>
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
          {onMove && (
            <button onClick={onMove} className="rounded-full bg-black/60 p-1.5 text-white hover:bg-black/80" title="Verplaats naar map">
              <FolderInput className="h-3.5 w-3.5" />
            </button>
          )}
          {url && (
            <a href={url} target="_blank" rel="noreferrer" className="rounded-full bg-black/60 p-1.5 text-white hover:bg-black/80" title="Openen">
              <Download className="h-3.5 w-3.5" />
            </a>
          )}
          <button onClick={onDelete} className="rounded-full bg-destructive/80 p-1.5 text-white hover:bg-destructive" title="Verwijderen">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-2">
        ...
        <div className="text-[10px] text-white/60">{new Date(u.created_at).toLocaleDateString("nl-NL")}</div>
      </div>
    </div>
  );
}
