...
function AdminTasks() {
  ...
  const counts = useMemo(() => {
    ...
    filtered.forEach((t: any) => { c[t.status as Status] = (c[t.status as Status] ?? 0) + 1; });
    return c;
  }, [filtered]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">ClickUp-stijl overzicht van alle klantentaken.</p>
      </div>
      ...
      <div className="glass-strong rounded-2xl p-4">
        <div className="grid gap-2 sm:grid-cols-[1fr_180px_140px_160px_auto]">
          ...
          />
          <select value={quickClient} onChange={(e) => setQuickClient(e.target.value)} className="rounded-lg bg-input/60 hairline px-3 py-2.5 text-sm">
            <option value="">Klant kiezen…</option>
            {clients?.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          <select value={quickPrio} onChange={(e) => setQuickPrio(e.target.value as Priority)} className="rounded-lg bg-input/60 hairline px-3 py-2.5 text-sm">
            <option value="low">Laag</option>
            <option value="medium">Normaal</option>
            <option value="high">Hoog</option>
            <option value="urgent">Urgent</option>
          </select>
          <input type="date" value={quickDue} onChange={(e) => setQuickDue(e.target.value)} className="rounded-lg bg-input/60 hairline px-3 py-2.5 text-sm" />
          <button onClick={quickAdd} className="rounded-lg bg-gradient-gold px-4 py-2.5 text-sm font-medium text-primary-foreground inline-flex items-center gap-1">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select value={filterClient} onChange={(e) => setFilterClient(e.target.value)} className="rounded-lg bg-input/60 hairline px-3 py-2 text-sm">
          ...
          return (
            <div key={col.k} className="glass rounded-2xl p-3 min-h-[300px]">
              ...
              <div className="space-y-2">
                {items.map((t: any) => (
                  <div key={t.id} className="rounded-xl bg-surface-elevated/60 p-3 border border-border/30 hover:border-gold/30 transition">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-medium leading-snug">{t.title}</div>
                        {t.description && <div className="text-xs text-muted-foreground mt-1 line-clamp-2">{t.description}</div>}
                      </div>
                      <button onClick={() => del(t.id)} className="text-muted-foreground hover:text-destructive shrink-0">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </div>
                    <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[10px]">
                      <span className="rounded-full bg-gold/15 text-gold px-2 py-0.5">{clientName(t.client_id)}</span>
                      <span className={cn("rounded-full border px-2 py-0.5 inline-flex items-center gap-1", PRIO_META[t.priority as Priority] ?? PRIO_META.medium)}>
                      ...
                      {t.due_date && (
                        <span className="rounded-full border border-border/40 text-muted-foreground px-2 py-0.5 inline-flex items-center gap-1">
                          <Calendar className="h-2.5 w-2.5" /> {new Date(t.due_date).toLocaleDateString("nl-NL", { day: "numeric", month: "short" })}
                        </span>
                      )}
                    </div>
                    <div className="mt-2 flex gap-1">
                      {STATUS_COLS.filter((c) => c.k !== col.k).map((c) => (
                        <button key={c.k} onClick={() => setStatus(t.id, c.k)} className="text-[10px] rounded px-2 py-0.5 bg-accent/40 hover:bg-gold/15 hover:text-gold transition">
                          → {c.label}
                        </button>
                      ))}
                    </div>
                ...
                {items.length === 0 && (
                  <div className="text-xs text-muted-foreground text-center py-8 border border-dashed border-border/30 rounded-xl">Leeg</div>
                )}
              </div>
            </div>
          );
