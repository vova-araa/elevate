...
import { Instagram, Plus, Trash2, Clock, AlertCircle, CheckCircle2, Loader2, Image as ImageIcon, Upload } from "lucide-react";
...
export function InstagramScheduler({ clientId, igUrl }: { clientId: string; igUrl: string | null }) {
  ...
  const { data: connection } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("social_connections" as any).select("*").eq("client_id", clientId).eq("platform", "instagram").maybeSingle()).data,
  });

  const { data: posts } = useQuery<Post[]>({
    queryKey: ["scheduled-posts", clientId],
    queryFn: async () =>
      ((await supabase.from("scheduled_posts" as any).select("*").eq("client_id", clientId).order("scheduled_at", { ascending: true })).data ?? []) as any,
  });
  ...
  async function connectInstagram() {
    ...
    const handle = prompt("Instagram-gebruikersnaam (zonder @)", igUrl?.split("/").filter(Boolean).pop() ?? "");
    if (!handle) { setConnecting(false); return; }
  ...
  async function disconnect() {
    ...
    await supabase.from("social_connections" as any).delete().eq("id", (connection as any).id);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass-strong rounded-2xl p-5 flex items-center gap-4">
        ...
        {connection ? (
          <button onClick={disconnect} className="text-xs text-destructive hover:underline">Loskoppelen</button>
        ) : (
      ...
      {planned.length > 0 && (
        <div className="space-y-2">
          <div className="text-xs uppercase tracking-[0.2em] text-gold/80">Gepland ({planned.length})</div>
          <div className="space-y-2">
            {planned.map((p, i) => (
...
function PlannedTile({ post }: { post: Post }) {
  ...
  useEffect(() => {
    ...
    supabase.storage.from("social-media").createSignedUrl(post.media_path, 3600).then(({ data }) => {
      if (data?.signedUrl) setUrl(data.signedUrl);
    });
  }, [post.media_path]);
  return (
    <div className="aspect-square relative ring-2 ring-gold/80 ring-inset bg-surface-elevated/60">
      {url ? <img src={url} alt="" className="h-full w-full object-cover" /> : <div className="h-full w-full bg-gradient-to-br from-gold/20 to-gold/5" />}
      <div className="absolute top-1 left-1 inline-flex items-center gap-1 rounded-full bg-black/70 px-1.5 py-0.5 text-[9px] text-gold">
        <Clock className="h-2.5 w-2.5" /> {new Date(post.scheduled_at).toLocaleDateString("nl-NL", { day: "numeric", month: "short" })}
      </div>
    </div>
  );
}
...
function PublishedTile({ post }: { post: Post }) {
  ...
  return (
    <div className="aspect-square relative bg-surface-elevated/60">
      {url ? <img src={url} alt="" className="h-full w-full object-cover" /> : <div className="h-full w-full bg-muted/30" />}
...
function PlannedRow({ post, allPlanned, index }: { post: Post; allPlanned: Post[]; index: number }) {
  const qc = useQueryClient();
  ...
  async function changeTime(value: string) {
    ...
    await supabase.from("scheduled_posts" as any).update({ scheduled_at: iso }).eq("id", post.id);
    qc.invalidateQueries({ queryKey: ["scheduled-posts", post.client_id] });
  }
  async function del() {
    if (!confirm("Geplande post verwijderen?")) return;
    await supabase.from("scheduled_posts" as any).delete().eq("id", post.id);
    qc.invalidateQueries({ queryKey: ["scheduled-posts", post.client_id] });
  }
  async function swap(otherId: string) {
    ...
    await supabase.from("scheduled_posts" as any).update({ scheduled_at: other.scheduled_at }).eq("id", post.id);
    await supabase.from("scheduled_posts" as any).update({ scheduled_at: post.scheduled_at }).eq("id", other.id);
    qc.invalidateQueries({ queryKey: ["scheduled-posts", post.client_id] });
    toast.success("Postmoment geruild");
  }
  async function markPublished() {
    await supabase.from("scheduled_posts" as any).update({ status: "published", published_at: new Date().toISOString() }).eq("id", post.id);
    qc.invalidateQueries({ queryKey: ["scheduled-posts", post.client_id] });
  }

  const dt = new Date(post.scheduled_at);
  const inputValue = new Date(dt.getTime() - dt.getTimezoneOffset() * 60000).toISOString().slice(0, 16);

  return (
    <div
      className={cn("glass rounded-2xl p-3 flex gap-3 items-center cursor-grab active:cursor-grabbing",
        post.status === "failed" && "ring-1 ring-destructive/40")}
      draggable
      onDragStart={(e) => e.dataTransfer.setData("text/plain", post.id)}
      onDragOver={(e) => e.preventDefault()}
      onDrop={(e) => { e.preventDefault(); swap(e.dataTransfer.getData("text/plain")); }}
    >
      <div className="h-14 w-14 rounded-lg overflow-hidden shrink-0 ring-1 ring-gold/40 bg-surface-elevated/60">
        {thumb ? <img src={thumb} className="h-full w-full object-cover" alt="" /> : <ImageIcon className="h-5 w-5 m-auto text-muted-foreground mt-4" />}
      </div>
      <div className="flex-1 min-w-0 space-y-1">
        <div className="text-sm truncate">{post.caption || <span className="text-muted-foreground italic">geen caption</span>}</div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {post.error_message && <span className="text-[10px] text-destructive">{post.error_message}</span>}
        </div>
      </div>
      <div className="flex flex-col gap-1 shrink-0">
        <button onClick={markPublished} className="text-[10px] text-emerald-400 hover:underline">Gepubliceerd</button>
        <button onClick={del} className="text-muted-foreground hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
      </div>
    </div>
  );
}
  ...
  return (
    <form onSubmit={submit} className="glass rounded-2xl p-4 space-y-3">
      ...
      <div className="flex gap-2">
        ...
        />
        <button disabled={busy} className="inline-flex items-center gap-2 rounded-lg bg-gradient-gold px-4 py-2 text-sm font-medium text-primary-foreground">
          {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Plannen
        </button>
      </div>
    </form>
  );
