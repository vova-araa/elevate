 ...
 */
export function PostChip({ post, brandColor, variant = "compact", onDragStart, onDragEnd, onOpen }: {
  post: any;
  ...
  const time = new Date(post.scheduled_at).toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit" });
  ...
  if (variant === "roomy") {
    return (
      <button
        ...
        title={draggable ? "Sleep naar een andere dag om te verplaatsen" : "Gepubliceerd — niet versleepbaar"}
        className={cn(
  ...
  return (
    <button
      ...
      style={{ borderLeftWidth: 3, borderLeftColor: accent }}>
      {meta && <meta.Icon className="h-3 w-3 shrink-0" />}
      <span className="truncate">{time} {post.caption?.slice(0, 24) || "Geen caption"}</span>
    </button>
  );
}
