 ...
 */
export function WeekView({ cursor, byDay, brandColor, onClickDay, onDropPost, dragId, setDragId, onOpenPost }: {
  cursor: Date;
          ...
          return (
            <div key={k}
              ...
              <div className={cn("text-center pb-2", isToday && "text-gold")}>
                <div className="uppercase tracking-wider text-[10px] text-muted-foreground">{format(d, "EEE", { locale: nl })}</div>
                <div className={cn(
              ...
              <div className="space-y-1.5 flex-1">
                {items.map((p: any) => (
                  <PostChip key={p.id} post={p} brandColor={brandColor} variant="roomy"
                    onDragStart={() => setDragId(p.id)} onDragEnd={() => setDragId(null)}
                    onOpen={() => onOpenPost(p.id)} />
                ))}
              </div>
              <button onClick={() => onClickDay(d)}
