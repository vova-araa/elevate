 ...
 */
export function ClientLegend({ clients, activeId, onSelect }: {
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {clients.map((c) => (
        <button key={c.id} type="button" onClick={() => onSelect(c.id)}
          ...
          <span className="h-2.5 w-2.5 rounded-full shrink-0" style={{ backgroundColor: c.brand_color || GOLD_FALLBACK }} />
          {c.name}
        </button>
      ))}
    </div>
