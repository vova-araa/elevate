...
export function ClientTimeline({ clientId }: { clientId: string }) {
  ...
  const { data: events = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [
        ...
        supabase
          .from("deals")
          .select("id,title,stage,value_cents,created_at,updated_at,closed_at")
        ...
        supabase
          .from("reports")
          .select("id,title,report_type,created_at,updated_at")
          .eq("client_id", clientId),
        supabase
          .from("evaluations")
          .select("id,title,score,period_label,created_at,updated_at")
          .eq("client_id", clientId),
        supabase
          .from("messages")
          .select("id,subject,body,sender_role,priority,created_at")
          .eq("client_id", clientId),
        supabase
          .from("meetings")
          .select("id,title,meeting_type,scheduled_at,created_at,updated_at")
          .eq("client_id", clientId),
        supabase
          .from("strategy_notes")
        ...
        supabase
          .from("calendar_items")
          .select("id,title,status,deliverable_type,date,created_at,updated_at")
          .eq("client_id", clientId),
        supabase
          .from("tasks")
      ...
      const pushPair = (
        ...
        if (
          updated &&
          updated !== created &&
          new Date(updated).getTime() - new Date(created).getTime() > 60_000
        ) {
          evts.push({
            id: `${kind}:${id}:u`,
            kind,
      ...
      deals.data?.forEach((d: any) =>
        pushPair(
          ...
          d.updated_at,
          d.title,
          `Stage: ${d.stage}${d.value_cents ? ` · €${(d.value_cents / 100).toLocaleString("nl-NL")}` : ""}`,
          d.stage,
        ),
      );
      ...
      evals.data?.forEach((e: any) =>
        pushPair(
          ...
          e.updated_at,
          e.title,
          `${e.period_label || ""}${e.score ? ` · score ${e.score}/10` : ""}`,
      ...
      messages.data?.forEach((m: any) =>
        evts.push({
          ...
          at: m.created_at,
          title:
            m.subject || (m.sender_role === "admin" ? "Bericht naar klant" : "Bericht van klant"),
      ...
      meetings.data?.forEach((m: any) =>
        pushPair(
          ...
          m.updated_at,
          m.title,
          `${m.meeting_type} · ${fmt(m.scheduled_at)}`,
          m.meeting_type,
        ),
      );
      ...
      calendar.data?.forEach((c: any) =>
        pushPair(
          ...
          c.updated_at,
          c.title,
          `${c.deliverable_type} · ${new Date(c.date).toLocaleDateString("nl-NL")}`,
          c.status,
        ),
      );
      ...
      if (rmIds.length) {
        ...
        rs?.forEach((s: any) =>
          pushPair(
            ...
            s.updated_at,
            s.title,
            `${s.deliverable_type || "stap"}${s.due_date ? ` · due ${new Date(s.due_date).toLocaleDateString("nl-NL")}` : ""}`,
            s.status,
          ),
        );
      }
  ...
  const filtered = useMemo(() => {
    ...
    return events.filter(
      (e) =>
        (filter === "all" || e.kind === filter) && (range === "all" || +new Date(e.at) >= cutoff),
    );
  }, [events, filter, range]);
    ...
    <div className="space-y-5">
        ...
        <div className="ml-auto inline-flex items-center gap-1 text-xs">
          ...
          {(["7", "30", "90", "all"] as const).map((r) => (
            <button
              ...
              onClick={() => setRange(r)}
              className={cn(
                "rounded-full px-3 py-1 hairline",
                range === r
      ...
      <div className="flex flex-wrap gap-1.5">
        <button
          ...
          className={cn(
            "inline-flex items-center gap-2 rounded-full hairline px-3 py-1.5 text-xs",
            filter === "all"
              ? "bg-gold/15 text-gold"
              : "text-muted-foreground hover:text-foreground",
          )}
        ...
        {(Object.keys(KIND_META) as EventKind[]).map((k) => {
          ...
          return (
            <button
              ...
              onClick={() => setFilter(k)}
              className={cn(
                "inline-flex items-center gap-2 rounded-full hairline px-3 py-1.5 text-xs transition",
              ...
              {items.map((e) => {
                ...
                return (
                  <li key={e.id} className="pl-6 relative">
                    <span
                      className={cn(
                        "absolute -left-[11px] top-1.5 grid h-5 w-5 place-items-center rounded-full border",
                        M.color,
                      )}
                    >
