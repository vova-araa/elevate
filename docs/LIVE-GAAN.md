...
> zijn alleen _waar de app draait_. Ken je Render al (bv. van truckandtrailer.nl)?
...
Er zijn zes nieuwe database-migraties. Open je Supabase-project →
...
5. `supabase/migrations/20260719120000_metrics_snapshots.sql` — volgers-historie voor echte groeicijfers
6. `supabase/migrations/20260720120000_intake_strategy.sql` — klant-intake en AI-strategie
...
2. Instellingen:
   ...
   - **Instance Type**: minstens **Starter** (~$7/mnd). _Let op:_ de gratis
...
4. Klik **Create Web Service**. Na de build heb je een adres als
   `https://elevate.onrender.com`. Eigen domein koppelen kan via _Settings →
   Custom Domains_.
...
3. Zet de gekregen codes bij de env-variabelen van je host
   (Vercel → _Settings → Environment Variables_, of Render → _Environment_) en
...
account tijdelijk toe als _tester_ in het developer-portaal, of dien de review
